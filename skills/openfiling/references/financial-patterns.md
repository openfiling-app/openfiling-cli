...
Example patterns for retrieving and analyzing financial data with the OpenFiling CLI. These are starting points — adapt the `jq` filters to suit your specific analysis needs.
...
openfiling financial income-statement 7203 --from-year 2020 --to-year 2024

# Revenue trend across periods
openfiling financial income-statement 7203 --from-year 2020 | \
...
openfiling financial balance-sheet 7203 --from-year 2020 | \
...
```bash
openfiling financial summary 7203 --from-year 2022
...
# → [{"basis":"consolidated_us_gaap",...,"availableFromYear":2012,"availableToYear":2020},
#    {"basis":"consolidated_ifrs",...,"availableFromYear":2017,"availableToYear":2025,"isDefault":true}]

# IFRS income statement
openfiling financial income-statement 7203 --basis consolidated_ifrs

# US-GAAP era data
openfiling financial income-statement 7203 --basis consolidated_us_gaap --from-year 2015 --to-year 2020
```
...
openfiling financial income-statement 7203 --basis consolidated_us_gaap --from-year 2016 --to-year 2017
openfiling financial income-statement 7203 --basis consolidated_ifrs --from-year 2017 --to-year 2018
```
...
```bash
# All ratios
openfiling analysis ratios 7203 --from-year 2020
...
openfiling analysis valuation 7203 --from-year 2020
...
openfiling financial equity 7203 --from-year 2022 | \
...
openfiling corporate segments 7203 --from-year 2022

# Available segment keys
openfiling corporate segments 7203 | jq '.segments[] | {key, label}'

# Segment detail with performance data
openfiling corporate segment 7203 JapanSegment --from-year 2022

# Extract revenue from a specific segment
openfiling corporate segment 7203 JapanSegment | \
  jq '[.periods[] | {fiscalYear, revenue: (.lineItems[] | select(.label | contains("売上")) | .value)}]'
```
...
# Non-consolidated
openfiling financial income-statement 7203 --basis non_consolidated_jgaap
```
...
for ticker in 7203 7267 7201; do
  echo "=== $ticker ==="
  openfiling analysis ratios $ticker --from-year 2023 | \
    jq '[.periods[] | {fiscalYear, roe: (.lineItems[] | select(.abbreviation == "ROE") | .value)}]'
done
```
